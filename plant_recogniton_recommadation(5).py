...
def hybrid_recommendation(user_features, user_ratings=None, top_n=80):
    ...
    # 在排序逻辑后添加分数增强
        ...
        # 新增分数增强逻辑（对特征匹配度高的结果加权）
        result_df['predicted_rating'] = result_df.apply(
            lambda x: min(x['predicted_rating'] * 1.2, 5.0) if x['feature_score'] >= 10 else x['predicted_rating'],
            axis=1
        )
        return result_df
...
with tab2:
                    ...
                    for idx, row in st.session_state.result_df.head(6).iterrows():
                        with st.expander(f"🌴 {row['plant']} | 预测评分: {row['predicted_rating']:.1f}/5", expanded=True):
                            ...
                            rating = st.slider(
                                "请为本项推荐评分",
                                1, 5, 4,  # 默认值改为4
                                key=f"plant_rating_{row['plant']}_{idx}",  # 添加索引保证唯一性
                                help="5分表示非常满意，1分表示不满意"
                            )
    ...
    # 步骤5：推荐完成
        ...
        if 'satisfaction_result' in st.session_state:
            ...
            cols = st.columns([1, 3])
            with cols[0]:
                st.metric("用户评分满意度", f"{satisfaction:.1f}%")  # 修改指标名称
